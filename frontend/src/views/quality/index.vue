...
const fieldStats = computed(() => stats.value?.fields[field.value] ?? null)
  ...
  <div class="p-4">
    <n-card :bordered="false" class="mb-4">
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <n-tag v-if="stats" :bordered="false" type="info">
          S+A 占比 {{ (stats.sa_ratio.value * 100).toFixed(1) }}% / 目标 20%
        </n-tag>
        <span v-if="stats" style="color: #999; font-size: 12px">{{ stats.note }}</span>
        <div class="flex-1" />
        <n-button quaternary :loading="loading" @click="refreshAll">
          刷新
        </n-button>
      </div>
    </n-card>
    ...
    <!-- 指标卡：三准确率 vs 目标线 -->
    <n-grid v-if="stats" :x-gap="12" :y-gap="12" :cols="3" class="mb-4" responsive="screen" item-responsive>
      <n-gi v-for="(f, key) in stats.fields" :key="key" span="3 m:1">
    ...
    <n-card :bordered="false">
      ...
      <n-empty v-if="!items.length && !loading" description="该维度暂无可抽检线索（或已全部检完）" />
      <n-space v-else vertical size="large">
        <n-card v-for="item in items" :key="item.lead_id" size="small" bordered>
          <div class="flex items-center gap-2 flex-wrap">
            <n-tag size="small" :type="item.grade === 'S' ? 'error' : item.grade === 'A' ? 'warning' : 'default'">
              {{ item.grade }} · {{ item.score }}
            </n-tag>
            <n-button size="small" quaternary type="primary" @click="router.push(`/collect/lead/${item.lead_id}`)">
              {{ item.name }}
            </n-button>
          ...
          <ul style="margin: 8px 0 10px; padding-left: 18px; color: #666; font-size: 13px">
            <li v-for="(line, i) in evidenceLines(item)" :key="i">
              {{ line }}
            </li>
          </ul>
          <div class="flex items-center gap-2 flex-wrap">
            <n-input
              ...
              style="width: 260px"
            />
            <n-button size="small" type="success" secondary @click="mark(item, 'correct')">
              ✓ 判定正确
            </n-button>
            <n-button size="small" type="error" secondary @click="mark(item, 'incorrect')">
              ✗ 判定错误
            </n-button>
            <n-button size="small" quaternary @click="mark(item, 'unsure')">
              ? 无法判定
            </n-button>
          </div>
        </n-card>
