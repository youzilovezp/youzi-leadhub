...
async def test_seed_business_once_and_skip(db_session, monkeypatch):
    ...
    # 前置：清空 leads 及子表（共享库无级联，显式删防行 id 复用串分）
    for table in (LeadFollowUp, LeadEvent, LeadSignal, LeadContact):
    ...
    assert dups == 0
    # 全部在 ICP 门内（终态池导出：qualified 才留池）
    icp = dict(
        (
            await db_session.execute(
                select(Lead.icp_status, func.count()).group_by(Lead.icp_status)
            )
        ).all()
    )
    assert set(icp) == {"qualified"}
    # 任务按 collector 建齐
    collectors = {c for (c,) in (await db_session.execute(select(CollectTask.collector))).all()}
