...
**线索采集服务控制台** — 为销售获取「需要用 WhatsApp 做生意」的企业商机线索
...
- **多源采集**（插件式采集器，注册即接入任务/去重/评分体系）
  - `google_maps` — Google Places API 按「关键词 × 城市」采集商家
  - `job_posting` — 招聘站点监控（kalibrr 等），在招 WhatsApp 客服的公司即高意向线索
  - `website_enrich` — 富化存量线索：抓取官网检测 WhatsApp 插件/链接、公开邮箱、社媒主页
  - 手工录入（同样走去重合并）
- **去重合并**（系统核心）：电话 E.164 / 域名 registrable domain / 公司名归一化，三身份列反查跨来源合并到同一 Lead
- **意向评分**：7 个布尔信号加权（WhatsApp 插件 +40、在招 WhatsApp 岗位 +30 …），权重可 env 覆盖
- **任务体系**：DB 即队列（无 Celery/Redis），支持并发闸门、取消、进度/日志实时轮询、APScheduler cron 定时
- **控制台 UI**：线索筛选（国家/行业/来源/分数/WhatsApp 检测/关键词）、勾选批量检测 WhatsApp、任务创建表单按 param_schema 动态渲染、日志流式查看
...
| 采集 | Crawlee（HTTP 爬虫）、httpx[socks]、phonenumbers、tldextract、APScheduler 3.x |
| 前端 | Vue 3 + TypeScript + Vite + Naive UI + Tailwind（原子类布局） |
| 中间件 | Docker Compose（PostgreSQL / Redis / adminer），优先复用本机已运行实例 |
...
| 账号 | `admin` / 见 `backend/.env` 的 `INITIAL_ADMIN_PASSWORD`（默认 `admin`） | **生产前必须改密码** |
...
去重、评分、任务调度、日志、前端表单全部自动接入。

## 文档导航

- [业务逻辑梳理](docs/业务逻辑.md) — 数据流 / 去重合并 / 评分 / 任务生命周期 / 设计取舍
- [脚手架使用说明](项目说明.md) — 启动、命令、加业务模块
- [AGENTS.md](AGENTS.md) — AI 助手开发约定
- 后端：[架构](backend/docs/架构说明.md) · [配置](backend/docs/配置说明.md) · [开发](backend/docs/开发指南.md) · [API](backend/docs/API文档.md)
- 前端：[架构](frontend/docs/架构说明.md) · [配置](frontend/docs/配置说明.md) · [开发](frontend/docs/开发指南.md)
